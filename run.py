...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="eachlabs-daily-") as tmp:
        ...
        if existing:
            ...
            print(f"  ⚠ Same-named report already in Drive — no new data since last run.")
            print(f"  Skipping Slack notification to avoid re-announcing an old report: {file_link}")
        else:
            ...
            # 4. Post to Slack — only when we actually uploaded something new
